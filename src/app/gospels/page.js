...
import { parables, getParablesByGospel } from "@/data/parables";
import { miracles, getMiraclesByGospel } from "@/data/miracles";
...
export default function GospelsPage() {
  const gospelBooks = gospelIds.map((id) =>
    books.newTestament.find((b) => b.id === id)
  );

  const parableCounts = {};
  const miracleCounts = {};
  gospelIds.forEach((id) => {
    parableCounts[id] = getParablesByGospel(id).length;
    miracleCounts[id] = getMiraclesByGospel(id).length;
  });
